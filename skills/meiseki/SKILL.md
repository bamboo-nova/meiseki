...
description: >
  ...
  二重否定・長い連体修飾・「の」の連鎖・過剰な名詞化・冗長表現・埋もれた列挙といった
...
### Step 4. patterns.md の A→F に沿ってリライトする
`references/patterns.md` を参照し、**優先度 A→F の順**で直す。

- **A（否定・条件の入れ子）が最優先。** 二重否定は最初に畳む。
- textlint が拾った箇所（A 二重否定 / B 一文長・読点 / C 連続漢字・冗長 / D 接続詞重複・弱い表現）は
  その指摘を起点に直す。
- **textlint が拾えない構文は LLM 側で判断する**：B 長い連体修飾、C「の」連鎖の深さ・一般の名詞化、
  E 体裁、F 埋もれた列挙の箇条書き化。ここが meiseki の独自価値であり、textlint 設定だけでは到達できない。
...
- **textlint で決定論的に取れる**：A 二重否定、B 一文長・読点・逆接「が」連続、C 連続漢字・「することができる」、
  D 冗長・接続詞重複・弱い表現。
- **textlint では取れない（LLM が担う）**：B 長い連体修飾、C「の」連鎖の深さ・一般の名詞化、E 体裁、F 列挙の箇条書き化。
